...
AHRS::AHRS(){
  ...
  float GyroMeasError = M_PI * (3.0f / 180.0f);     // gyroscope measurement error in rads/s (start at 60 deg/s), then reduce after ~10 s to 3
...
void StartIMUTask(void const *argument) {
  ...
  for (;;) {
    ...
    mpu->readRawData();
    ahrs->MadgwickQuaternionUpdate(mpu->data.ax, mpu->data.ay, mpu->data.az,
